...
#grant all privileges on `bank`.* to 'jsa7868'@'%'; #대표계정에도 권한 주기
#grant all privileges on `shop`.* to 'jsa7868'@'%'; 
#flush privileges
...
insert into`bank_account`(`a_no`,`a_item_dist`,`a_item_name`,`a_c_no`,`a_balance`,`a_open_date`)values('101-23-1007','L3','주택담보대출','750210-1000002',2700000,'2020-09-23');
...
#실습 2-20 복습하기
select count(if(`t_dist`=1,1,null)) as '입금 건수', count(if(`t_dist`=2,1,null)) as '출금 건수', count(if(`t_dist`=3,1,null)) as '조회 건수'
...
#실습 2-26
select `a_no` as '계좌번호', `a_item_name` as '계좌이름',`a_c_no` as '주민번호(사업자번호)',
`c_name` as '고객명', `a_balance` as '현재잔액'

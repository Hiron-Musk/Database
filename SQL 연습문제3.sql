...
#GRANT ALL PRIVILEGES ON College.* TO 'jsa7868'@'%';
#FLUSH PRIVILEGES;
...
insert into `lecture`(`lecno`,`lecname`,`leccredit`,`lectime`,`lecclass`)values(167,'운영체제론',3,25,'본805');
...
insert into `lecture`(`lecno`,`lecname`,`leccredit`,`lectime`,`lecclass`)values(248,'빅데이터 개론',3,20,'본801');
insert into `lecture`(`lecno`,`lecname`,`leccredit`,`lectime`,`lecclass`)values(253,'컴퓨팅사고와 코딩',2,10,'본802');
...
on a.reglecno=b.lecno where `lecno`=349;
...
#실습3-28
select `stdno`,`stdname`,`lecno`,`lecname`,`regtotalscore`,`reggrade`
from `student` as a
join `register` as b on a.stdno=b.regstdno
join `lecture` as c on b.reglecno=c.lecno
where `reggrade`='F';
